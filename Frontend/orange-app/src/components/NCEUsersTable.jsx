...
const NCEUsersTable = () => {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const response = await axios.get('http://localhost:5254/api/UserNCEImport');
      setUsers(response.data);
      setError(null);
    } catch (err) {
      setError('Error loading NCE users');
  ...
  const handlePasswordUpdate = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:5254/api/UserNCEImport/${passwordUser.id}/password`, {
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:5254/api/UserNCEImport', newUser);
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:5254/api/UserNCEImport/${editingUser.id}`, editingUser);
  ...
  const handleSuspend = async (id) => {
    if (window.confirm('Êtes-vous sûr de vouloir suspendre cet utilisateur?')) {
      try {
        await axios.put(`http://localhost:5254/api/UserNCEImport/${id}/suspend`);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Êtes-vous sûr de vouloir supprimer cet utilisateur? Cette action est irréversible.')) {
      try {
        await axios.delete(`http://localhost:5254/api/UserNCEImport/${id}`);
  ...
  return (
    <div className="overflow-x-auto bg-white rounded-lg shadow-lg p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-800">NCE Users Management</h2>
        <button 
          ...
          {users.length > 0 ? (
            users.map((user) => (
              <tr key={user.id} className="hover:bg-orange-50 transition duration-150 ease-in-out">
                ...
                <td className="py-3 px-4 text-left text-gray-700 font-medium">{user.nceUserName}</td>
      ...
      {showPasswordModal && passwordUser && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-8 w-full max-w-md shadow-2xl transform transition-all">
            ...
            <p className="mb-6 text-gray-600">Changing password for user: <strong className="text-gray-800">{passwordUser.nceUserName}</strong></p>
